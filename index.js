...
if (autobio === 'true'){ 
            setInterval(() => { 
                         ...
                         client.updateProfileStatus( 
                                         ...
                                         `${botname} is active 24/7\n\n${date.toLocaleString('en-US', { timeZone: 'Africa/Accra' })} It's a ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Nairobi'})}.` 
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.badSession) {
        ...
        startDreaded();
                 ...
                 await client.groupAcceptInvite("ChtNjeh19AR1Nw0omweVvV");


        console.log(`✅ ALPHA connection successful\nLoaded ${totalCommands} commands.\nBot is active.`);


        const getGreeting = () => {
            const currentHour = DateTime.now().setZone('Africa/Accra').hour;
...
message += `📝 𝑪𝑶𝑴𝑴𝑨𝑵𝑫𝑺:- ${totalCommands}\n`
        message += '🕝 𝑻𝑰𝑴𝑬:- ' + getCurrentTimeInAccra() + '\n';
...
app.listen(port, () => console.log(`ALPHA Server listening on port http://localhost:${port}`));
